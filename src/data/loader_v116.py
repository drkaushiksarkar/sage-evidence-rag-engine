"""LoaderV116: loader_v116 module v2.
...
@dataclass
class LoaderV116Config:
    """Configuration for loader_v116 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV116(nn.Module):
    """Neural network module for loader_v116 v2.
    ...
    def __init__(self, config: Optional[LoaderV116Config] = None):
        super().__init__()
        self.config = config or LoaderV116Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV116 v2 with %d params",
