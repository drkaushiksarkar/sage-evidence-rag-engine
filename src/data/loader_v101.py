"""LoaderV101: loader_v101 module v2.
...
@dataclass
class LoaderV101Config:
    """Configuration for loader_v101 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV101(nn.Module):
    """Neural network module for loader_v101 v2.
    ...
    def __init__(self, config: Optional[LoaderV101Config] = None):
        super().__init__()
        self.config = config or LoaderV101Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV101 v2 with %d params",
