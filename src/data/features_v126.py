"""FeaturesV126: features_v126 module v1.
...
@dataclass
class FeaturesV126Config:
    """Configuration for features_v126 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class FeaturesV126(nn.Module):
    """Neural network module for features_v126 v1.
    ...
    def __init__(self, config: Optional[FeaturesV126Config] = None):
        super().__init__()
        self.config = config or FeaturesV126Config()
        c = self.config
        ...
        logger.info("Initialized FeaturesV126 v1 with %d params",
