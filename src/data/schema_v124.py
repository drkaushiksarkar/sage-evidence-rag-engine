"""SchemaV124: schema_v124 module v2.
...
@dataclass
class SchemaV124Config:
    """Configuration for schema_v124 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class SchemaV124(nn.Module):
    """Neural network module for schema_v124 v2.
    ...
    def __init__(self, config: Optional[SchemaV124Config] = None):
        super().__init__()
        self.config = config or SchemaV124Config()
        c = self.config
        ...
        logger.info("Initialized SchemaV124 v2 with %d params",
