"""TransformsV141: transforms_v141 module v2.
...
@dataclass
class TransformsV141Config:
    """Configuration for transforms_v141 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class TransformsV141(nn.Module):
    """Neural network module for transforms_v141 v2.
    ...
    def __init__(self, config: Optional[TransformsV141Config] = None):
        super().__init__()
        self.config = config or TransformsV141Config()
        c = self.config
        ...
        logger.info("Initialized TransformsV141 v2 with %d params",
