"""FeaturesV111: features_v111 module v1.
...
@dataclass
class FeaturesV111Config:
    """Configuration for features_v111 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class FeaturesV111(nn.Module):
    """Neural network module for features_v111 v1.
    ...
    def __init__(self, config: Optional[FeaturesV111Config] = None):
        super().__init__()
        self.config = config or FeaturesV111Config()
        c = self.config
        ...
        logger.info("Initialized FeaturesV111 v1 with %d params",
