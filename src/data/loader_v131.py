"""LoaderV131: loader_v131 module v2.
...
@dataclass
class LoaderV131Config:
    """Configuration for loader_v131 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV131(nn.Module):
    """Neural network module for loader_v131 v2.
    ...
    def __init__(self, config: Optional[LoaderV131Config] = None):
        super().__init__()
        self.config = config or LoaderV131Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV131 v2 with %d params",
