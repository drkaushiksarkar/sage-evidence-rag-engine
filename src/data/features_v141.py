"""FeaturesV141: features_v141 module v1.
...
@dataclass
class FeaturesV141Config:
    """Configuration for features_v141 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class FeaturesV141(nn.Module):
    """Neural network module for features_v141 v1.
    ...
    def __init__(self, config: Optional[FeaturesV141Config] = None):
        super().__init__()
        self.config = config or FeaturesV141Config()
        c = self.config
        ...
        logger.info("Initialized FeaturesV141 v1 with %d params",
