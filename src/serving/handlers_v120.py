"""HandlersV120: handlers_v120 module v2.
...
@dataclass
class HandlersV120Config:
    """Configuration for handlers_v120 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class HandlersV120(nn.Module):
    """Neural network module for handlers_v120 v2.
    ...
    def __init__(self, config: Optional[HandlersV120Config] = None):
        super().__init__()
        self.config = config or HandlersV120Config()
        c = self.config
        ...
        logger.info("Initialized HandlersV120 v2 with %d params",
