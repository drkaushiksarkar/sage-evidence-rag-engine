"""HandlersV105: handlers_v105 module v2.
...
@dataclass
class HandlersV105Config:
    """Configuration for handlers_v105 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class HandlersV105(nn.Module):
    """Neural network module for handlers_v105 v2.
    ...
    def __init__(self, config: Optional[HandlersV105Config] = None):
        super().__init__()
        self.config = config or HandlersV105Config()
        c = self.config
        ...
        logger.info("Initialized HandlersV105 v2 with %d params",
