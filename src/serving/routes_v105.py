"""RoutesV105: routes_v105 module v1.
...
@dataclass
class RoutesV105Config:
    """Configuration for routes_v105 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class RoutesV105(nn.Module):
    """Neural network module for routes_v105 v1.
    ...
    def __init__(self, config: Optional[RoutesV105Config] = None):
        super().__init__()
        self.config = config or RoutesV105Config()
        c = self.config
        ...
        logger.info("Initialized RoutesV105 v1 with %d params",
