"""RoutesV120: routes_v120 module v1.
...
@dataclass
class RoutesV120Config:
    """Configuration for routes_v120 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class RoutesV120(nn.Module):
    """Neural network module for routes_v120 v1.
    ...
    def __init__(self, config: Optional[RoutesV120Config] = None):
        super().__init__()
        self.config = config or RoutesV120Config()
        c = self.config
        ...
        logger.info("Initialized RoutesV120 v1 with %d params",
