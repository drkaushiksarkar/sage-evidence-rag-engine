...
@dataclass
class ArchV102Config:
    """Configuration for arch_v102 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class ArchV102(nn.Module):
    """Neural network module for arch_v102 v1.
    ...
    def __init__(self, config: Optional[ArchV102Config] = None):
        super().__init__()
        self.config = config or ArchV102Config()
        c = self.config
        ...
        logger.info("Initialized ArchV102 v1 with %d params",
