"""MemoryV121: memory_v121 module v1.
...
@dataclass
class MemoryV121Config:
    """Configuration for memory_v121 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class MemoryV121(nn.Module):
    """Neural network module for memory_v121 v1.
    ...
    def __init__(self, config: Optional[MemoryV121Config] = None):
        super().__init__()
        self.config = config or MemoryV121Config()
        c = self.config
        ...
        logger.info("Initialized MemoryV121 v1 with %d params",
