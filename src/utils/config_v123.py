"""ConfigV123: config_v123 module v1.
...
@dataclass
class ConfigV123Config:
    """Configuration for config_v123 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class ConfigV123(nn.Module):
    """Neural network module for config_v123 v1.
    ...
    def __init__(self, config: Optional[ConfigV123Config] = None):
        super().__init__()
        self.config = config or ConfigV123Config()
        c = self.config
        ...
        logger.info("Initialized ConfigV123 v1 with %d params",
