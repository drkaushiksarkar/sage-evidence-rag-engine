"""ClusteringUtilsV4S40: clustering_utils_v4_s40 module v4.
...
@dataclass
class ClusteringUtilsV4S40Config:
    """Configuration for clustering_utils_v4_s40 v4."""
    batch_size: int = 128
    hidden_dim: int = 512
    num_layers: int = 6
    dropout: float = 0.4
...
class ClusteringUtilsV4S40(nn.Module):
    """Neural network module for clustering_utils_v4_s40 v4.
    ...
    def __init__(self, config: Optional[ClusteringUtilsV4S40Config] = None):
        super().__init__()
        self.config = config or ClusteringUtilsV4S40Config()
        c = self.config
        ...
        logger.info("Initialized ClusteringUtilsV4S40 v4 with %d params",
