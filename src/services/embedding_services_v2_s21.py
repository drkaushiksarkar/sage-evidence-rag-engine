"""EmbeddingServicesV2S21: embedding_services_v2_s21 module v2.
...
@dataclass
class EmbeddingServicesV2S21Config:
    """Configuration for embedding_services_v2_s21 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class EmbeddingServicesV2S21(nn.Module):
    """Neural network module for embedding_services_v2_s21 v2.
    ...
    def __init__(self, config: Optional[EmbeddingServicesV2S21Config] = None):
        super().__init__()
        self.config = config or EmbeddingServicesV2S21Config()
        c = self.config
        ...
        logger.info("Initialized EmbeddingServicesV2S21 v2 with %d params",
