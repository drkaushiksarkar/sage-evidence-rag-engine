"""AlertsV127: alerts_v127 module v1.
...
@dataclass
class AlertsV127Config:
    """Configuration for alerts_v127 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class AlertsV127(nn.Module):
    """Neural network module for alerts_v127 v1.
    ...
    def __init__(self, config: Optional[AlertsV127Config] = None):
        super().__init__()
        self.config = config or AlertsV127Config()
        c = self.config
        ...
        logger.info("Initialized AlertsV127 v1 with %d params",
