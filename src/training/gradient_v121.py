"""GradientV121: gradient_v121 module v2.
...
@dataclass
class GradientV121Config:
    """Configuration for gradient_v121 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class GradientV121(nn.Module):
    """Neural network module for gradient_v121 v2.
    ...
    def __init__(self, config: Optional[GradientV121Config] = None):
        super().__init__()
        self.config = config or GradientV121Config()
        c = self.config
        ...
        logger.info("Initialized GradientV121 v2 with %d params",
