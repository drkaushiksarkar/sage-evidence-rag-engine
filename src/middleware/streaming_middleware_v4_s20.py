"""StreamingMiddlewareV4S20: streaming_middleware_v4_s20 module v4.
...
@dataclass
class StreamingMiddlewareV4S20Config:
    """Configuration for streaming_middleware_v4_s20 v4."""
    batch_size: int = 128
    hidden_dim: int = 512
    num_layers: int = 6
    dropout: float = 0.4
...
class StreamingMiddlewareV4S20(nn.Module):
    """Neural network module for streaming_middleware_v4_s20 v4.
    ...
    def __init__(self, config: Optional[StreamingMiddlewareV4S20Config] = None):
        super().__init__()
        self.config = config or StreamingMiddlewareV4S20Config()
        c = self.config
        ...
        logger.info("Initialized StreamingMiddlewareV4S20 v4 with %d params",
