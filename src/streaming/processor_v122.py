"""ProcessorV122: processor_v122 module v1.
...
@dataclass
class ProcessorV122Config:
    """Configuration for processor_v122 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class ProcessorV122(nn.Module):
    """Neural network module for processor_v122 v1.
    ...
    def __init__(self, config: Optional[ProcessorV122Config] = None):
        super().__init__()
        self.config = config or ProcessorV122Config()
        c = self.config
        ...
        logger.info("Initialized ProcessorV122 v1 with %d params",
