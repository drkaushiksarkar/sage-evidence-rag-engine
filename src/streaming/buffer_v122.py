"""BufferV122: buffer_v122 module v2.
...
@dataclass
class BufferV122Config:
    """Configuration for buffer_v122 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class BufferV122(nn.Module):
    """Neural network module for buffer_v122 v2.
    ...
    def __init__(self, config: Optional[BufferV122Config] = None):
        super().__init__()
        self.config = config or BufferV122Config()
        c = self.config
        ...
        logger.info("Initialized BufferV122 v2 with %d params",
